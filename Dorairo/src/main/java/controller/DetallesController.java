...
/**
 * Clase Detalles que nos ayuda a ver toda la información de las peliculas y de
 * las series
 * 
 * @author Doriana dc
 */
public class DetallesController {
	...
	/**
	 * Maneja el evento de clic en la opción de inicio. Configura la escena y el
	 * escenario y muestra la ventana de perfil.
	 * 
	...
	/**
	 * Maneja el evento de clic en la opción de película. Configura la escena y el
	 * escenario y muestra la ventana de perfil.
	 * 
	 * @param event El evento de clic del mouse.
	 * 
	...
	/**
	 * Maneja el evento de clic en la opción de serie. Configura la escena y el
	 * escenario y muestra la ventana de perfil.
	 * 
	 * @param event El evento de clic del mouse.
	 * 
	...
	/**
	 * 
	 * Maneja el evento de clic en la opción de buscador. Configura la escena y el
	 * escenario y muestra la ventana de perfil.
	 * 
	 * @param event El evento de clic del mouse.
	 * 
	...
	/**
	 * Maneja el evento de clic en la opción de perfil. Configura la escena y el
	 * escenario y muestra la ventana de perfil.
	 * 
	 * @param event El evento de clic del mouse.
	 * 
	...
	private void addYaVistas() {
		if (tipo.equals(Constants.PELICULA)) {
			...
			UsuarioPelicula usuarioPelicula = upDao.searchByUsuarioAndMovieId(usuario.getUser(), pelicula.getId());
			if (usuarioPelicula != null && usuarioPelicula.isVista()) {
				...
				UsuarioPelicula listaPersonalizada = upDao.searchByUsuarioAndMovieId(usuario.getUser(), pelicula.getId());
				...
				UsuarioPelicula up = new UsuarioPelicula(new UsuarioPeliculaID(usuario, pelicula), 0, null, null, null, true,
						false);
				upDao.update(up);
			...
			UsuarioSerie usuarioSerie = usDao.searchByUsuarioAndSerieId(usuario.getUser(), series.getId());
			if (usuarioSerie != null && usuarioSerie.isVista()) {
				...
				UsuarioSerie listaPersonalizada = usDao.searchByUsuarioAndSerieId(usuario.getUser(), series.getId());
				...
				UsuarioSerie us = new UsuarioSerie(new UsuarioSerieID(usuario, series), 0, null, null, null, true, false);
				usDao.update(us);
				Utils.mostrarAlerta("¡Serie agregada a ya vistas correctamente!", Constants.INFORMATION_TYPE);
			}
		}
	}
	...
	private void addMiLista() {
		if (tipo.equals(Constants.PELICULA)) {
			...
			if (usuarioPelicula != null && usuarioPelicula.isMiLista()) {
				...
				UsuarioPelicula yaVista = upDao.searchByUsuarioAndMovieId(usuario.getUser(), pelicula.getId());
				...
				UsuarioPelicula up = new UsuarioPelicula(new UsuarioPeliculaID(usuario, pelicula), 0, null, null, null, false,
						true);
			...
			if (usuarioSerie != null && usuarioSerie.isMiLista()) {
				...
				UsuarioSerie us = new UsuarioSerie(new UsuarioSerieID(usuario, series), 0, null, null, null, false, true);
				usDao.update(us);
	...
	/**
	 * Inicializa los datos de la ventana de detalles de una película o serie según
	 * su tipo y ID.
	 * 
		...
		if (id.startsWith(Constants.PREFIJO_ID_PELIS_MANUALES)) {
			...
			if (pelicula != null) {
				...
				detalles.setText("Descripción: " + pelicula.getOverview() + "\n" + "Fecha de estreno: "
						...
						+ formatActores(pelicula.getActores()) + "\n" + "Directores: " + formatDirectores(pelicula.getDirectores())
						+ "\n" + "Géneros: " + formatGeneros(pelicula.getGenres()) + "\n" + "Compañía: "
				...
				Utils.mostrarAlerta("Error al mostrar los detalles. Película no encontrada.", Constants.ERROR_TYPE);
			}
			...
			if (series != null) {
				...
				Utils.mostrarAlerta("Error al mostrar los detalles. Serie no encontrada.", Constants.ERROR_TYPE);
			}
		} else {
			mostrarDatosApi();
		}
	...
	/**
	 * 
	 * Establece la película actual para mostrar sus detalles. * @param pelicula La
	 * película a establecer.
	 */
	...
	private void obtenerActoresYDirectores(String id) {
		...
		Request creditsRequest = new Request.Builder().url(creditsUrl).get().addHeader("accept", "application/json")
				.addHeader("Authorization", "Bearer " + API_KEY).build();
		try (Response creditsResponse = client.newCall(creditsRequest).execute()) {
			...
			JsonArray cast = creditsJson.has("cast") ? creditsJson.getAsJsonArray("cast") : new JsonArray();
			JsonArray crew = creditsJson.has("crew") ? creditsJson.getAsJsonArray("crew") : new JsonArray();
		...
		if (!comentarioTexto.isBlank()) {
			...
			if (tipo.equals(Constants.PELICULA)) {
				...
				if (usuarioPelicula == null) {
					...
					usuarioPelicula = new UsuarioPelicula(new UsuarioPeliculaID(usuario, pelicula), 0, new Date(), null, null,
							false, false);
				}
				...
				if (usuarioSerie == null) {
					...
					usuarioSerie = new UsuarioSerie(new UsuarioSerieID(usuario, series), 0, new Date(), null, null, false, false);
				}
	...
	/**
	 * Método para cargar el comentario desde la base de datos y mostrarlo en la
	 * ventana
	 */
	...
	private void searchLocalizacion() {
		...
		if (guardadoEn.getText().isBlank()) {
			...
			if (tipo.equals(Constants.PELICULA)) {
				...
				UsuarioPeliculaDaoImpl upDao = new UsuarioPeliculaDaoImpl(session);
				UsuarioPelicula up = upDao.searchByUsuarioAndMovieId(UsuarioController.getUsuarioRegistrado().getUser(),
						pelicula.getId());
				// Comprueba si es null
				if (up == null) {
					setPelicula(gePeliculaBBDD());
					// Crea un nuevo UsuarioPelicula con todos los datos a null
					up = new UsuarioPelicula(new UsuarioPeliculaID(usuario, pelicula), 0, new Date(), null, null, false, false);
				}
				...
				if (us == null) {
					setSeries(getSerieBBDD());
					// Crea un nuevo UsuarioSerie con todos los datos a null
					us = new UsuarioSerie(new UsuarioSerieID(usuario, series), 0, new Date(), null, null, false, false);
				}
				// Establece la localizacion para esa Pelicula
